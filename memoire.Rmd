...
udfig <- function(l=" ",s=" ",sep=" - ",...){
  opts_chunk$set(fig.cap=paste(l),fig.scap=paste(l,sep,s),boite=paste(l),out.lines=10)
  }
...
knit_hooks$set(source = function(x, options) {
  ...
  if (!is.null(n <- options$out.lines)) {
    x <- xfun::split_lines(gsub("#", "\\ttvar{#}",x))
    if (length(x) > n) {
      x <- c(head(x, n), "....\n")
...
print(data[0]["fields"]["geom"]["coordinates"])
```
...
udfig(l=l,s=s)
```
```{r io_gpd, echo=FALSE, fig.align = 'center', out.width = "90%"}
...
Ensuite, **un calque par hauteur** sera créé en amont des tracés des volumes, de sorte à ce qu'ils
...
et **à l'échelle** (puisque ces coordonnées sont exprimés en mètres).
\
...
Une première étape consiste à créer un **nouveau document rhino**, spécifier ses unités (ici, en accord avec les unités des coordonnées *Lambert93*, soit le *mètre*),
...
\
\
En outre, tous les scripts présentés permettent d’obtenir chacun de ces éléments en **un temps d'éxécution très faible (quelques minutes tout au plus)**, 
...
udfig(l=l,s=s)
```
```{r mats, echo=FALSE, fig.align = 'center', out.height = "30%",fig.show='hold'}
...
```{r echo=FALSE}
l = "Principe de l'aggrégation géométrique"
s = "Réalisation personnelle"
udfig(l=l,s=s)
```
```{r principe_agg, echo=FALSE, fig.align = 'center', out.width = "45%",fig.show='hold'}
include_graphics("__imgs/principe_agg.png")
```

En l'absence d'identifiant en commun, il est nécessaire de réaliser une agrégation de données basées sur les **coordonnées** 
de deux entités identiques appartenant à des jeux différents.
Dans un premier temps, elle sera appliquée entre les jeux de données de l'**APUR** et de la **BD TOPO**. L'extraction des images sera quant à lui 
...
pour plusieurs raions telles que la précision du relevé ou encore une éventuelle différence de mises à jour.
...
En l'occurence, puisque le jeu de données de l'**APUR** contient la majorité des données à extraire, c'est celui de la
