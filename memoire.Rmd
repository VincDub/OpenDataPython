---
title: |
  ![](__imgs/couverture.png){width=88%}
  \thispagestyle{empty}
  \newpage
fontsize : 11pt
...
knit_hooks$set(source = function(x, options) {
  ...
  if (options$echo != FALSE || options$fig.align == "center") {
    if (options$fig.align == "center") {
      if (before) {paste("\\begin{tcolorbox}[title=\\begin{customfigs}",options$boite,"\\end{customfigs}]\\masked{\\captionof{figure}{",options$fig.scap,"}}")} else {paste("\\end{tcolorbox}\n")}
      } 
      else {
...
s = "https://www.youtube.com/watch?v=kJ_TXZB4Gm4"
udfig(l=l,s=s)
```
