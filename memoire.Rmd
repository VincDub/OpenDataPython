...
lang: fr-FR
no-cite: ["@iavulga","@intelarchi","@ptarchisuper"]
indent: true
---
...
udfig <- function(l=" ",s=" ",sep=" - ",...){
  opts_chunk$set(fig.cap=NULL,collapse=TRUE,fig.scap=paste(l,sep,s),boite=paste(l),out.lines=10,dpi=150)
}
...
knit_hooks$set(source = function(x, options) {
  ...
  if (options$echo != FALSE || options$fig.align == "center") {
    if (options$fig.align == "center") {
      if (before) {paste("\\vskip 11pt\n\\begin{tcolorbox}[title=\\begin{customfigs}",options$boite,"\\end{customfigs}]\\masked{\\captionof{figure}{",options$fig.scap,"}}")} else {paste("\\end{tcolorbox}\n\\vskip 11pt\n")}
      } 
      else {
        if (before) {paste("\\vskip 11pt\n\\begin{tcolorbox}[title=",options$boite,",colback=boitecode]\n")} else {paste("\\end{tcolorbox}\n\\vskip 11pt\n")}
      }
    }})
...
\begin{titlepage}
\includepdf[pages={1},pagecommand={\thispagestyle{empty}}]{__imgs/couverture.pdf}
\end{titlepage}
...
\
\
Plus précisément, cet ouvrage se concentrera sur l'emploi du **langage Python** avec lequel j'ai eu plusieurs fois l'occasion de travailler au cours de mon cursus, choisi ici pour sa syntaxe claire et simplifiée. 
Le but sera ici de répondre aux principaux enjeux autour de l’appropriation de ces données ouvertes par les architectes,
à travers une **approche pratique constituée de plusieurs scripts Python**. 
En quoi un langage comme Python est-il indispensable aujourd'hui pour exploiter les données en Open Data ? De quelle manière peut-on les intégrer dans l’environnement de travail de la conception 
...
En effet, c’est avant tout une stratégie prônant l’ouverture du plus grand nombre de bases de données au public, les rendant ainsi les plus accessibles possibles. 
À l’instar des autres mouvements du même type, tel que « l’Open Source », le traitement et la rediffusion des données sont autorisés, voir même, comme c’est le cas par 
le gouvernement français : **« les données partagées trouvent des ré utilisateurs qui les intègrent dans de nouveaux services à forte valeur ajoutée économique ou sociale. »**.
\
\
...
Cependant, une donnée accessible en Open Data ne provient pas forcément d'un organisme public, comme le prouve la société *Uber*,
mettant à disposition publiquement des données anonymisées sur ses taxis [@uberopen].
\
\
...
\
\
Ainsi, sur le territoire Français, des dispositifs mis en oeuvre par le gouvernement tel qu’« Etalab », chargé de la coordination et la mise en place de l’ouverture de 
...
De nombreuses plateformes déployées par diverses instances opérant dans des domaines très variés ont vu le jour au cours des dernières années, allant d’organismes spécialisés dans 
...
\
\
Bien que cette nécessité étatique de partager l’information publique ne date pas de l’apparition du Web (comme l’explique la loi Cada de 1978), 
ce dernier a permis, en plus d'une **dispense de tout intermédiaire** (notamment humain) **entre le fournisseur et l’utilisateur**, 
d’exploiter de nouvelles formes d’accès, en particulier grâce à **des scripts ou des algorithmes** écrits dans un langage de programmation 
afin d’**automatiser la récupération de données depuis les formats de fichiers ouverts**.
\
\
Ainsi, quiconque cherche à mettre en place un travail d’analyse le plus exhaustif possible d’un contexte donné peut, 
grâce aux plateformes et moyens cités ci-dessus, **disposer très rapidement de données riches et abondantes**.
\
\
De ce point de vue là, il paraît extrêmement pertinent pour les métiers issus de l’architecture et de l’urbanisme, 
et en particulier le **métier d’architecte**, de se saisir des données issues de l’Open Data afin de **renforcer leur compréhension du territoire sur lequel ils construisent**, 
que cela soit par la simple analyse statistique ou bien la récupération d’informations géométriques d’un site.
\
\
Or, les architectes ont tendance à préférer, par leur expertise orientée sur la conception réclamant un esprit de synthèse affûté, 
les résultats explicites d’analyse de données plutôt que les données en elles-mêmes. De plus, 
les outils numériques sur lesquels les architectes se forment relèvent très majoritairement des domaines du dessin, de la modélisation ou de la communication plutôt que de l’analyse, ce 
qui accentue leur dépendance à des résultats synthétiques « pré-établis ».
\
\
Cependant, il existe depuis les années 2010 un certain **essor des travaux** basés sur des données issues en partie ou totalement de l’Open Data, et ce, grâce à un **langage de programmation** en particulier, 
dont la **simplicité de la syntaxe** couplée à une **profusion de bibliothèques** (comparables à des « plug-in ») spécialisées dans le traitement de données informatiques en ont fait un **outil populaire pour la recherche** d’aujourd’hui, le **Python**.
À juste titre, ce langage est désormais très répandu au sein des Systèmes d'Informations géographiques (SIG) tels que ArcGIS, 
où ses caractéristiques mentionnées ci-dessus permettent de manière accessible de mener des travaux complexes autour des données géographiques ouvertes, 
de l'analyse et la visualisation [@arcgis1] à l'entraînement de modèles de prédiction [@arcgis2].
\
\
Comme l’illustre le projet « CityEngine - Twitter » mené au « Centre for Advanced Spatial Analysis » de Londres [@hugel_cityengine-twitter_2014], 
proposant une cartographie urbaine de densité basée sur des Tweets géolocalisés dans cette même ville, **un seul et unique script** en Python permet à la fois de récupérer les messages sur une plage de 24 heures 
(via une bibliothèque , nommée « Tweepy » , permettant au code de récolter les données de Twitter), de les trier et d’en extraire leurs coordonnées et leur horaire de publication, 
et enfin de fournir ces informations directement à l’outil de génération de modèles 3D urbains « CityEngine » (publié par l’ESRI) afin que ce dernier puisse constituer une carte procédurale 
(animée selon le nombre de "tweets" sur une plage de 24 heures).
\
\
...
\
\
Une telle étude étant désormais possible sur des données plus restreintes, elle peut encore plus aisément être mis en place lorsque les données utilisées sont totalement ouvertes et avec accès illimité.
Ainsi, grâce à des données massives et accessibles (tant en termes de tarifs que de facilité d’extraction) couplées à un langage de programmation comme Python, 
développer ses propres protocoles d'analyse par exploitation de données brutes est désormais à la portée des chercheurs, sans avoir besoin d’un bagage informatique conséquent.
\
\
Dès lors, face à la **complexité des enjeux** auxquels la **conception architecturale** est confrontée (climatique, socio-économique, écologique ou structurel par exemple), il semble pertinent d’envisager que des architectes se saisissent de ce type d’outil, dans le but de construire, 
au prisme de leur propre volonté d’intervention (même complexes), leurs **propres modèles de compréhension du territoire**.
Ce nouveau regard, personnalisé par l’architecte, pourrait alors apporter à ce dernier des éléments susceptibles de le guider de manière bien plus significative, en particulier au cours des premières phases d’esquisse, afin d’**améliorer la qualité de sa production**. 
\
\
...
\newline
\
\normalfont\color{black}Après avoir initialement démontré l’intérêt d'un langage de programmation comme Python dans l’extraction et la manipulation des données issues des plateformes accessibles en Open Data, un aperçu de l'aptitude de ce langage à produire des documents de travail utiles à l’architecte sera livré (cartographie, dessin et modélisation) à travers plusieurs scripts. Enfin, ce travail d’exploitation sera abouti en montrant la prodigieuse capacité du langage Python à permettre de manière accessible l’analyse complexe de ces données ainsi que la mise en place d’algorithmes de prédiction.

\newpage

# La programmation : outil d'exploitation privilégié de l'Open Data

Tel que le stipule le portail européen des données, au-delà de l’accessibilité en elle-même, la question de la lisibilité des structures de données et des formats de fichiers disponibles sur les plateformes en Open Data est d’importance cruciale : « On peut utiliser des données, car elles sont disponibles sous une forme commune et lisibles par des machines. »[@opendataeu]. Cet organisme relève également un autre aspect primordial, celui de la facilité du traitement des données par les outils informatiques. En effet, elles ont davantage vocation à faire l’objet de manipulations automatiques (synthèse, tri, etc.) plutôt que d’être simplement lues par un utilisateur humain.
\
\
Pour partager des données tabulaires (sous forme de tableur) par exemple, là où un utilisateur humain préfèrera un format Excel (.XLSX) (en y incluant notamment couleurs et styles de polices pour améliorer sa lisibilité), le portail européen des données recommande d’autres formats comme le .CSV (Comma Separated Values), format ouvert constitué de texte brut séparé par des caractères spéciaux, compatible avec un large panel d’outils logiciels capables d’opérations de traitement. 
\
\
Face à ce besoin de compréhension et de manipulation de données brutes, les langages de programmation de haut niveau d’abstraction (possédant une syntaxe plus claire et concise pour l’humain, rendant leur appropriation accessible) et en particulier le Python apparaissent alors comme des outils offrant les caractéristiques nécessaires pour répondre à cette problématique.
\
\
...
La plupart des plateformes distribuant des données en Open Data proposant directement en ligne des  interfaces de prévisualisation, 
ces dernières constituent un excellent outil de lecture initiale.
C'est le cas sur le site Open Data Paris, qui nous permet de prévisualiser le jeu de données des volumes bâtis sous la forme d'un tableau, mais aussi d'une carte,
...
\
\
En tant que lecture complémentaire, il paraît intéressant de consulter  également la prévisualisation sous forme de tableau afin d'apporter une vue plus claire des différentes variables et leurs valeurs.
...
informatique. Elles semblent **catégoriques**, c'est-à-dire ne pouvant prendre qu'un nombre défini de valeurs possibles.
...
\
\
À l'inverse, d'autres variables comme *B_RDC* ne possèdent ni un nom explicite ni une valeur permettant de suggérer sa signification. Elle semble toutefois catégorique, cette fois-ci exprimée
...
à l'utilisateur  les explications nécessaires à la compréhension des variables constituant
...
Premièrement, ce document explique sa logique de séparer un bâtiment « réel » en plusieurs volumes fictifs, suivants s'ils sont 
...
obligeant à le sectionner si l'on veut y faire figurer de manière correcte un patio par exemple). 
...
ses différentes valeurs possibles si ces dernières sont prédéfinies (servant à distinguer les variables **catégoriques**).
\
Dès lors, il est aisé de repérer les variables les plus pertinentes si l'on souhaite extraire la hauteur des différents volumes.
En l'occurrence, ce sont les variables **H_ET_MAX** ainsi que **L_B_U** (pour les volumes en porte à faux), toutes deux exprimées en nombre d'étages.
...
Ainsi, une **brève entrevue** sera ici apportée sur chacun de ces formats grâce à Python, afin de déterminer lequel choisir.
\
...
un objet simulant un tableau nommé *DataFrame*, composé de lignes et de colonnes.
...
Cette dernière repose sur la **notation objet**, une syntaxe commune à de nombreux langages de programmation modernes comme le Python.
...
permettant d'y effectuer des opérations spécifiques.
...
Pour toutes ces qualités, cette approche avec **GeoPandas** sera privilégiée à travers les chapitres suivants.
...
d'effectuer une multiplication. Cependant, l'attribut **hauteur_paf** étant noté sous la forme de **texte** (chaînes de caractères) décrivant les plages de hauteur,
...
Heureusement, Python est capable de reconnaître des morceaux de texte au sein de chaînes de caractères, permettant ainsi de les remplacer par un équivalent numérique.
...
Dès lors, afin de comprendre les différentes manières d'exprimer les hauteurs en porte à faux, il est possible de les énumérer par **nombre de caractères** de chacune des notations.
\
\
...
# et changement de la valeur associée à "B"
print(d["A"])
...
permettant d'opérer sur chacun d'entre eux automatiquement. 
\
\
...
Dès lors, il est nécessaire **ajouter 1 à tout nombre de niveaux sauf R** avant multiplication par la hauteur d'étage type.
...
Le code ci-dessous effectue la transformation d'une notation **N^1^aN^2^**. Après s'être assuré que la notation contient
...
Elle appliquera également la transformation de la variable **hauteur** (à savoir un incrément de 1 suivi d'une multiplication par la hauteur d'étage type) évoquée en début de section.
...
\
\
En effet, c’est à travers ce type de document que l’architecte est capable de 
...
offrant alors énormément de possibilités de représentation, du graphique statistique au modèle 3D.
\
\
**Ce chapitre présentera ainsi sous forme de scripts plusieurs variantes d’exploitation des données récoltées à la fin du chapitre précédent, dans le but d’obtenir des documents 
synthétiques. 
Elles seront respectivement consacrées à l’élaboration de graphiques statistiques, 
puis d’une carte interactive, d’un dessin vectorisé avec gestion des calques, et enfin à un modèle 3D du contexte bâti.**
...
Ici, deux exports seront effectués, à savoir un export **statique** sous forme d'image au format PNG, ainsi qu'un export **interactif** au sein d'une page web au format HTML. 
...
Il est également possible de créer de la même manière une multitude de représentations plus complexes.
Un second graphique sera donc construit, en s'intéressant cette fois-ci à une répartition **des volumes suivant leur hauteur, leur surface et s'ils sont en porte à faux**.
Ici, le sous-module *express* de *Plotly* sera employé permettant une mise en forme plus condensée,
l'objectif étant de se baser directement sur le jeu de données des volumes. 
Cependant, une nouvelle colonne devra être ajoutée, spécifiant si chaque volume est en porte à faux ou pas,
afin de pouvoir renseigner cette valeur dans *Plotly*.
...
En l'occurrence, cette  visualisation permet d'analyser plus finement le jeu de données.  Ce dernier contient alors davantage de petites surfaces ainsi qu'un nombre équilibré de volumes en porte à faux.
Cependant, cette souplesse de la part de *Plotly* dans la création de représentations graphiques
rend surtout possible de dessiner les **emprises des volumes en eux-mêmes**, le tout pouvant être également augmenté avec de l'interactivité.
...
En effet, *Plotly* est capable de tracer des **formes géométriques géoréférencées sur un fond de carte** 
...
en configurant le titre, la légende ainsi que le style de fond de carte. Le tout est sauvegardé au format .HTML afin de l’ouvrir dans un navigateur permettant les options d’interactivité et la **navigation libre** dans le fond de carte.
...
Dès lors, une telle aptitude à cartographier avec souplesse des données brutes présente un intérêt certain
...
Chacun d’entre eux sera également classé dans un **calque correspondant à sa hauteur**, avec pour chacun d'entre eux une couleur différente.
Pour ce faire, le module **ezdxf** sera employé. Ce dernier permet la plupart des fonctions de dessin vectoriel
que proposent d'autres outils de CAD tels qu'*AutoCAD*, dont la création de calques entre autres.
\
\
...
Elle se révèle donc essentielle pour pouvoir dessiner dans un repère tel que celui d'un dessin vectorisé.
...
apparaissent dans un ordre croissant au sein du futur fichier. Tout comme dans la carte interactive présentée dans la section
2.1.2, une **teinte de couleur proportionnelle à chaque hauteur** sera créée (en l'occurrence, sur une échelle de rouge).
...
Ceci effectué, une boucle itère à travers chaque volume du jeu de données (grâce à la fonction *iterrows()* de *GeoPandas*).
...
vectoriels, pleinement exploitables comme support de travail.

\newpage

### Construction d’un modèle 3D

Cette sous-section aura pour but de clore le chapitre en exploitant à son plein potentiel le jeu de données extrait.
...
\
\
De plus, même si cette latence a tendance à diminuer avec leur évolution, la quasi-totalité des bibliothèques orientées 3D pour Python sont basées sur des versions codées dans des langages de programmation plus complexes
...
Python (permettant tout de même toute opération possible manuellement dans le logiciel).
...
Une première étape consiste à créer un **nouveau document de Rhinocéros**, puis en spécifier les unités (ici, en accord avec les unités des coordonnées *Lambert93*, soit le *mètre*) et enfin créer un calque dans lequel seront dessinés les volumes. Ce dernier se verra attribuer un **nom** et une **couleur**.
...
\
La première étape consiste à **extraire les points  définissants chaque polygone** et de les grouper au sein d'une liste (nommée *pts*) afin d'en générer une **polyligne**.
\
...
le langage Python apporte à l’architecte des solutions techniques variées afin de convertir les jeux de données brutes en 
...
peuvent être mises en place de manière concise (grâces aux bibliothèques comme *GeoPandas*), en prolongement des concepts abordés dans le chapitre 1.
\
\
...
Comme l'a montré le jeu des « volumes bâtis » au sein des chapitres précédents, les jeux de données rencontrés en 
...
Dès lors, il existe un réel enjeu à être en capacité de **recouper des jeux de données appartenant à des thématiques différentes**,
...
ces dernières ne proposent alors aucun dispositif (tel qu'un identifiant) permettant d'identifier ces mêmes entités de manière universelle. 
...
identifiant propre à la plateforme en question plutôt qu'un identifiant commun, ne permettant donc pas un recoupement par ce biais.
\
\
Heureusement, certaines bibliothèques Python comme **GeoPandas** (que j'ai eu l'occasion de présenter à plusieurs reprises au cours des 
...
jusqu'à une intégration au sein de plateformes majeures du secteur comme **TensorFlow**[@tensorflow] ou **PyTorch**[@pytorch] majoritairement basées sur ce langage, et proposant des 
fonctionnalités plus approfondies.
\
\
Ce chapitre présentera un exemple de travail d'**agrégation** de trois jeux de données provenant d'organismes différents (dans le but de n'en former qu'un seul),
caractérisant des bâtiments de Paris et sa petite couronne** sous plusieurs aspects :

* Des données relatives à la **typo morphologie** (nature, date de construction,hauteur, emprise géoréférencée, surface vitrée,etc.).
* Des informations relatives à leur **matérialité**
* Des **photographies** relativement récentes de ces bâtiments
...
Comme présenté en introduction, il paraît compliqué (même impossible) de trouver un jeu de données déjà constitué avec des critères aussi divers et précis que ceux évoqués précédemment.
...
qu'il sera nécessaire d'exploiter, ainsi que leurs caractéristiques.
...
Dès lors, je me suis intéressé de près à la plateforme Open Data de l'**APUR** (Atelier parisien d’urbanisme) [@opendataapur], inscrit
dans le contexte géographique souhaité, à savoir le Grand Paris.
\
\
...
sont inclus, la plateforme de l'APUR permettant ici aussi une prévisualisation sous forme de carte.
...
Un autre jeu contenant des informations sur le bâti existe bel et bien sur une autre plateforme en Open Data mise en place par l'**IGN**, intitulé la **BD TOPO**.
Cette dernière est une base de données vectorielle à l'échelle nationale constituée de plusieurs **couches de données cartographiées et géoréférencées sur de multiples domaines**,
tels que les tracés des limites administratives, les infrastructures de transports, l'hydrographie et plus particulièrement **le bâti existant**.
...
udfig(l=l,s=s)
```
```{r mats, echo=FALSE, fig.align = 'center', out.width = "70%",fig.show='hold'}
include_graphics("__imgs/table_matmur.png")
```

Un petit script Python sera utilisé ci-dessous afin
de confirmer les attributs associés à un bâtiment grâce au module *GeoPandas* à partir du fichier des bâtiments au format ShapeFile 
contenu dans la base de données une fois téléchargée.
Un exemple d'identifiant est également extrait, montrant sa différence avec celui de l'exemple du jeu de l'*APUR* (noté sous la forme d'une seule suite de chiffres).
...
ajoutés au jeu de l'*APUR*.

\newpage

### IGN : Base de Données Orthographique (BD ORTHO)

Enfin, la dernière étape consiste à fournir des **photographies** récentes de chaque bâtiment. 
...
* De préférence, chaque bâtiment sujet doit pouvoir être **isolé** (sur un fond blanc ou noir par exemple) afin d'améliorer sa lisibilité.
...
L'enjeu du langage Python est donc ici de permettre ce **recoupement** de manière fiable et accessible.
...
en lui-même au maximum qui se répercutera positivement sur le temps d'exécution. 
...
\
\
Ainsi, pour les jeux de données de l'**APUR** et de la **BD TOPO**, une copie sera réalisée après cette phase d'apurement.
\
\
La bibliothèque *GeoPandas* facilite ici énormément ce type d'opération, en permettant d'inscrire directement
...
Concernant le jeu de données de l'**APUR**, après avoir changé son système de coordonnées de référence vers le système
...
bâtiment dont la date de construction est inconnue.
...
en appliquant un **dictionnaire** (associant chaque valeur et son équivalent textuel qui le remplacera) sur la colonne entière avec la fonction *.map()*.
...
L'apurement du jeu de données de la **BD TOPO** est quant à lui centré sur les deux attributs concernant les
matériaux. Dès lors, le code ci-dessus permet de s'assurer que tous les bâtiments possèdent bel et bien des informations
concernant leur **matérialité** (de sorte qu'elles soient définies). C'est pourquoi les valeurs *AUTRES* et *INDÉFINIES* sont supprimées.
Les bâtiments légers (non construits en « dur ») sont également éliminés.
\
\
...
**if** (si) et **elif** (sinon), contraction de *else* et *if*, affectent la bonne valeur au bon matériau.
Enfin, le jeu de données peut être sauvegardé.
\
\
Ainsi, la taille du jeu de données de l'**APUR** a pu être diminuée, passant de 1.6Go à 310Mo, idem pour le jeu de la **BD TOPO**, dont le volume est passé 3Go à 620Mo. 
Ces réductions de volume sont donc conséquentes, et permettront un gain notable
au niveau du temps d'exécution.
\
\
Une fois ces jeux de données réduits à leurs attributs les plus utiles, la phase de recoupement en elle-même peut être mise en oeuvre.
...
**BD TOPO** qui sera réduit.
\
\
Afin d'effectuer l'intersection des deux jeux de données, une seule fonction de *GeoPandas* sera employée, nommée *sjoin()* (spatial join).
Cependant, il est nécessaire que les deux jeux à recouper contiennent le même type de géométrie (ploygone, point, etc.).
...
très réduite, de sorte qu'il compte comme un polygone, sans impacter la méthodologie de départ (en restant un « point » à l'échelle d'un bâtiment). 
Ainsi, dans le code ci-dessous, un cercle de 10cm de rayon remplacera chaque polygone de la *BD TOPO*, construit à partir de leurs centroïdes respectifs.
...
Une fois cette étape effectuée, la seule commande *sjoin()* de *GeoPandas* suffit à effectuer l'agrégation des deux jeux,
en spécifiant l'argument *« inner »* (indiquant que l'on souhaite conserver uniquement l'intersection). 
...
dans la ligne suivante par une colonne *id*, numérotant simplement chaque bâtiment du nouveau jeu obtenu, qui 
peut enfin être sauvegardé. Un aperçu en est présenté dans le code ci-dessous, montrant la présence des attributs provenant des deux jeux pour un même bâtiment.
...
Dès lors, les dernières informations à obtenir afin de compléter ce jeu
...
aériennes.
Cependant, étant découpées en **carreaux géoréférencés**, il est nécessaire au préalable d'identifier quels bâtiments sont présents sur chacun de ces
carreaux.
\
\
Heureusement, *GeoPandas* permet de charger un jeu de données selon une *« Bounding Box »* définie par quatre points géoréférencés,
de sorte à ne charger que les entités inscrites au sein du rectangle décrit. 
...
Deux dossiers, l'un désignant l'emplacement où sont stockés les différents carreaux de la **BD ORTHO** et l'autre le répertoire d'enregistrement des images,
seront également référencés.
...
Cette transformation est effectuée par anticipation afin que ces images puissent être plus facilement traitées au sein d'algorithmes, tout en occupant un minimum de volume.
\
...
artificielle.
Ces exemples ont davantage pour but de démontrer la capacité des outils actuels permettant leur mise en oeuvre et leur exploitation 
...
Sa représentation graphique est une matrice **comportant les mêmes variables sur chaque dimension**, et  faisant figurer dans chaque case le **coefficient
de corrélation** (dans l'intervalle [-1,1]) entre les deux variables qui s'y croisent.
Afin de rendre cette matrice plus lisible, chaque coefficient peut être associé à une **couleur** suivant sa valeur, de sorte à créer un **corrélogramme** ou une
**Heatmap**.
\
\
L'objectif fixé ici sera d'identifier les liens entre **la nature d'un bâtiment** et ses **caractéristiques** au sein du jeu de données.
\
\
...
udfig(l=l,s=s)
```
```{r encode, echo=FALSE, fig.align = 'center', out.width = "70%",fig.show='hold'}
...
Dès lors, quelques observations peuvent être notées (relatives au contexte de référence, à savoir Paris et la Petite Couronne) :

* La typologie du bâtiment mixte ne possède quasiment aucun attribut avec laquelle elle serait fortement liée ou non,
sûrement dû à une forte **hétérogénéité** de sa population.

* Quant aux informations morphologiques, logement individuel et collectif s'opposent sans surprise, surtout au niveau de la hauteur, où les coefficients sont les plus extrêmes.
...
Comme mentionné dans l'introduction de ce chapitre, les jeux de données massifs agrégés représentent un potentiel majeur pour l'entraînement d'algorithmes prédictifs.
\
\
Ici, le but sera d'exploiter l'**imagerie aérienne** extraite afin d'entraîner un **réseau de neurones convolutif**,
capable d'identifier **le matériau de toiture d'un bâtiment donné**. Ce modèle de prédiction pourra être alors utilisé dans un autre contexte géographique.
\
\
Bien que cette section n'ait pas pour but de détailler le fonctionnement d'un tel réseau de neurones, le schéma ci-dessous présentera néanmoins son 
...
En effet, il sera démontré plus loin que le réseau en lui-même peut être traité comme une « boîte noire », le langage Python disposant
...
étant donné qu'un réseau de neurones n'accepte que ce type de valeur.
...
Tous les bâtiments sans images peuvent ainsi être éliminés facilement du jeu d'entraînement avec *.dropna()*.
...
Enfin, le modèle pourra être sauvegardé à l'issue de la séquence d'entraînement.
...
Ainsi, il est finalement possible de tester le modèle à partir d'une collection d'images constituée
de photographies aériennes de bâtiments provenant d'autres régions de France.
Ces images de test sont extraites du service en ligne *Géoportail*, détourées
...
compiler sa propre **base de données** pouvant être fondée sur des **thématiques multiples**, ce qui présente
...
prodiguant là aussi des outils comme **AutoKeras** destiné à **simplifier la création de modèles de prédiction** 
...
\
\
Au regard de ces outils pouvant d'ores et déjà être saisis par les architectes,
les **données disponibles actuellement** sont désormais le « réactif limitant » de leur exploitation de l'Open Data,
...
Grâce à l’approche pratique proposée au sein de ce mémoire, nous avons pu démontrer que le langage de programmation Python constitue un outil **souple et performant**, 
permettant d’accompagner l’architecte tout au long de son exploitation des données issues de l’Open Data. 
\
\
En effet, permettant en premier lieu de **rendre compréhensibles et manipulables aux plus novices les structures de données couramment rencontrées**, 
et en particulier les données géoréférencées, ce langage est doté de bibliothèques constituant des outils de production de documents synthétiques tout à fait capables de **représenter 
les données extraites dans des formats de fichiers courants pour l’architecte**, élément primordial au sein de son organisation. Ainsi, au-delà des graphiques statistiques, 
Python permet la génération de cartographies interactives, mais surtout des dessins techniques hiérarchisés et même des modèles 3D de manière automatique et personnalisée, représentant un **gain de temps** considérable.
\
\
Enfin, l’architecte devient capable grâce à Python de **mener ses propres travaux d'agrégation et d'exploitation approfondie** de jeux de données multi thèmes, 
afin de les **mettre à profit dans son activité de conception**, grâce à une capacité d’identification de corrélations entre ces différentes données agrégées, 
ou encore celle de pouvoir  entrainer ses propres modèles de prédiction ou de classification, grâce à une approche pratique simplifiée du Machine Learning.
\
\
...
le Python acquiert un véritable **caractère universel**, autant dans le sens où il est capable de se suffire à lui-même que pour qualifier sa compatibilité extraordinaire avec d’autres outils et services (comme Rhinocéros).
\
\
...
\
\
Premièrement, l’expérience acquise durant la manipulation des jeux de données ouverts (en particulier sur les notions des structures de données) 
sera extrêmement précieuse lorsqu’il s’agira d’exploiter des jeux de données plus directement liés à la pratique architecturale en elle-même, 
comme depuis un parc de « Smart Buildings » qu’il faudra surveiller et analyser par exemple.
\
\
Au-delà de cet aspect, des **solutions logicielles émergentes** telles que « Générative Design in Revit » publiée par Autodesk [@gdrevit] et désormais intégrée à *Revit 2021*
proposent déjà à l’architecte de travailler aux côtés d’algorithmes évolutifs et prédictifs, que ce soit de manière simplifiée par interface graphique ou bien personnalisable grâce à des 
langages de programmation visuelle comme Dynamo. La collaboration entre **agences d’architecture** et solutions intégrant de l’**Intelligence artificielle** commence également à se développer, 
tel que l'illustre l'exemple de l’agence *Valode&Pistre* et son partenariat avec « SpaceMaker AI» [@valodepistre], société norvégienne spécialisée dans la conception urbaine générative.
\
\
...
\
\
Toutes ces opportunités nouvelles requièrent une certaine **familiarité avec le fonctionnement algorithmique en général**, 
et surtout une assimilation de leur manière de fonctionner afin de les intégrer efficacement à son « workflow ».
\
\
Dès lors, par son statut de langage de programmation lui conférant un caractère universel quant aux notions de base informatiques et 
surtout algorithmiques (même avec une syntaxe simplifiée), le Python se présente comme un **atout « futur proof »** face à ces **nouvelles interactions architecte-machine**. De plus, 
l’adoption massive du Python par les architectes permettrait d’amorcer une **lutte contre la crainte de la montée en puissance de l’Intelligence artificielle** au sein des métiers du bâtiment, 
par une atténuation considérable de l’effet « Boite noire » qu’elle suscite. Ceci faciliterait donc grandement la transition du corps architectural dans la numérisation du domaine du bâtiment.
\
\
Au-delà de cette appropriation, une certaine motivation de créer des **outils logiciels par les architectes pour les architectes** intégrant ces nouvelles compétences pourrait émerger, 
permettant alors de réaffirmer la place du métier d’architecte au sein d’un écosystème de plus en plus techno centré.
...
udfig(l=l,s=s)
```
```{r pandoc, echo=FALSE, fig.align = 'center', out.width = "60%",fig.show='hold'}
...
Cependant, je souhaiterais explorer d'autres outils similaires moins répandus, mais plus minimalistes dans leur approche comme le **Groff** (intégré de base aux systèmes Linux), afin de déterminer quelle solution
est à la fois complète, simple et souple.

```{r echo=FALSE}
l = "Le Groff : alternative minimaliste intégrée aux systèmes Linux"
s = "\\url{https://www.youtube.com/watch?v=kJ_TXZB4Gm4}"
udfig(l=l,s=s)
```
```{r groff, echo=FALSE, fig.align = 'center', out.width = "90%",fig.show='hold'}
include_graphics("__imgs/groff.png")
```
...
La stratégie de l'\color{red}**Open Data**\color{black}, représentant l'initiative de la part d'institutions publiques et privées de partager leurs données
se déploie actuellement de manière accrue à travers la publication de \color{red}**jeux de données ouverts et massifs** \color{black} 
dans de multiples domaines.
Certains d'entre eux commencent à intégrer des \color{red}**données sur les bâtiments existants**\color{black}, des informations morphologiques à leurs matériaux de façade,
en passant par leurs besoins énergétiques, recelant alors un \color{red}**potentiel certain pour l'architecte**\color{black}.
\
\
Cependant, ces données étant avant tout \color{red}**formatées pour être manipulées de manière algorithmique**\color{black}, cela représente
un frein conséquent à leur exploitation par des acteurs peu familiers avec le domaine du traitement automatique de données.
\
\
Dès lors, les \color{red}**langages de programmation les plus accessibles**\color{black}, et en particulier le \color{red}**Python**\color{black}, très répandu dans
le domaine de la science des données tout en arborant une syntaxe simplifiée, se présentent comme une solution potentielle
permettant de \color{red}**fournir tous les outils nécessaires aux architectes afin d'exploiter les données
accessibles aujourd'hui en Open Data**\color{black}, en développant une approche \color{red}**concrète et pratique** \color{black} 
à l'aide de scripts restant \color{red}**clairs et concis**\color{black}.

\vspace*{\fill}

\center

![](__imgs/logo_vd.png){width=20%}
